...
2. Test the product and fix bugs, loading issues, and asset orientation or scale. Check cheap numeric signals first (counts, dimensions, file sizes): an unchanged number after an additive change means the change didn't land
3. Capture a screenshot for each view target, plus close-up crops for detail work — full views hide sub-10px errors
4. Judge each screenshot against its target, per [judge.md](judge.md)
5. Address all of the judge's feedback, largest gaps first
6. Test again, then check the exit criteria. When a new artifact appears, compare against the previous round's capture before blaming the latest change. If not done, return to step 3
...
- **Stall approaching**: the best score hasn't improved by a full point in 2 rounds, or the judge named the same gap twice in a row. Stop tweaking. Rethink the approach: assets, lighting, camera, or scene layout may be fundamentally wrong — a plateau within one stage usually means the stage is exhausted, so advance to the next stage instead of more tweaks. Aim for a dramatic improvement.
...
0. **Bundled assets.** Check the tool's own shipped library first (asset packs, material libraries, UV sets) — fitted stock beats hand-built geometry.

1. **Download** free assets, only if the user allowed it.
2. **Blender**, if installed or connected through MCP, for characters, buildings, scenery, greenery, and other detailed or important assets. Read [blender.md](blender.md) first — or [mpfb.md](mpfb.md) instead when the asset is a human and the MPFB add-on is available.
